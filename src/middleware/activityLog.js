/**
 * Traza legible de la actividad de la app: una linea por request con metodo,
 * ruta, estado y duracion. Pensada para seguir en vivo lo que hace un usuario
 * mientras navega (demos, grabaciones, diagnostico en produccion).
 *
...
const ACCIONES = [
  ...
  [/\/query-nlp\/preview/, '🔍 Búsqueda semántica'],
  [/\/chat\/search/, '🕘 Búsqueda en historial'],
  [/\/chat/, '🕘 Historial de consultas'],
  // Antes que las reglas de poha: /aporte/poha/atomic tambien contiene "poha".
  [/\/aporte/, '✍️  Aporte de contenido'],
  ...
  [/\/dolencias/, '🩺 Dolencias'],
  [/\/admin\/embeddings/, '⚙️  Regeneración de embeddings'],
...
function activityLog(req, res, next) {
  ...
  res.on('finish', () => {
    ...
    const estado = res.statusCode >= 500 ? '❌' : res.statusCode >= 400 ? '⚠️ ' : '✅';
    console.log(
